...
const $ = new Env('云模板每日签到');



const notify = $.isNode() ? require('./sendNotify') : '';
//Node.js用户请在jdCookie.js处填写京东ck;
const cookieNode = $.isNode() ? require('./22vdCookie.js') : '';
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.UserName = decodeURIComponent(cookie.match(/username=(.+?);/) && cookie.match(/username=(.+?);/)[1])
      $.index = i + 1;
      $.isLogin = true;
      $.nickName = '';
      $.message = '';
...
async function signDaySign(  ) {

	let url = 'https://www.22vd.com/wp-admin/admin-ajax.php';
	let body = 'action=sign_ajax&ajax_date=sign';
...
function safeGet(data) {
  try {
    ...
    console.log(e);
    console.log(`云模板服务器访问数据为空，请检查自身设备网络情况`);
    return false;
  }
}
