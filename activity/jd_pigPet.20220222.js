          ...
          if (data) {
            ...
            if (data.resultCode === 0) {
              if (data.resultData.resultCode === 0) {
                ...
                if (!$.hasPig) {
                  console.log(`\n京东账号${$.index} ${$.nickName} 未开启养猪活动,请手动去京东金融APP开启此活动或复制口令直达：\n27复制整段话 http:/J3RUwwnoKL4ACq 我的5斤百香果能领取啦，来养猪，一起赚復制整条信息￥2AbTu63jiDTSj7%打開—>最新版【亰 d0ng 金 融App】～\n`)
                  return
                }
            ...
            if (data.resultCode === 0) {
              if (data.resultData.resultCode === 0) {
                if (data.resultData.resultData) {
				  ...
				  updateAuthorShareCode(data.resultData.resultData.helpId);
...
function updateAuthorShareCode(code) {
	...
	let url = `${share_code_url}/jd_pig_pet/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
