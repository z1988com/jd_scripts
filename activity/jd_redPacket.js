...
!(async () => {
  ...
  let res = await getAuthorShareCode(`${share_code_url}/jd_red_packet`)
  if (!res) {
  ...
  if (res){
	$.redPacketId.push(res[0]); 
	if (res[1]) $.redPacketId.push(res[1]);		
	if (res[2]) $.redPacketId.push(res[2]);	
  }
...
async function red() {
  ...
  if ($.h5activityIndex && $.h5activityIndex.data && $.h5activityIndex.data.biz_code === 10002) {
    ...
    if (redPacketId) $.redPacketId.push(redPacketId);
  	//上传红包id
	  let res = updateRedPacketId(redPacketId);
        ...
        if (err) {
          ...
          if (data && data.data && data.data.biz_code === 0) {
            if (data.data.result.redPacketId) {
              ...
              //上传红包id
			        let res = updateRedPacketId( data.data.result.redPacketId );
...
function updateRedPacketId(redPacketId) {
	let key =  process.env.UPDATE_SHARE_CODE_KEY ?? '';
	let time = Date.now();
	let sign = md5(`username=${$.UserName}&red_packet_id=${redPacketId}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_red_packet/update?sign=${sign}&username=${$.UserName}&red_packet_id=${redPacketId}&time=${time}`;
    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
			...
			console.log(`updetaRedPacketId ：${data}`)
          if (data) data = JSON.parse(data)
