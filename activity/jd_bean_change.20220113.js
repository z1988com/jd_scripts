...
async function showMsg() {
  ...
  if (initPetTownRes && initPetTownRes.code === '0' && initPetTownRes.resultCode && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
...
async function bean() {
  ...
  do {
    ...
    } else if (response && response.code && response.code === "3") {
            ...
            try{
                if(err){
                    ...
                    if(data){
                        ...
                        if(data.retcode === "0" && data.data && data.data.hasOwnProperty("userInfo")){
                            $.nickName = `${data.data.userInfo.baseInfo.nickname} ${data.data.userInfo.baseInfo.levelName}`
                        }
                        if(data.retcode === '0' && data.data && data.data.assetInfo){
                            $.beanCount = data.data && data.data.assetInfo.beanNum;
                        }
    ...
    $.post(taskMsPostUrl('homePageV2', {}, 'appid=SecKill2020'), (err, resp, data) => {
      try {
        if (err) {
          ...
          if (safeGet(data)) {
			//console.log(`getMs: ${data}`);
            data = JSON.parse(data)
            if (data && data.code && data.code === 2041) {
