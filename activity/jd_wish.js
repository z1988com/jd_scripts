              ...
              for (let key of Object.keys(data.data.result.taskVos).reverse()) {
                ...
                if (vo.status !== 2) {
                  if (vo.taskType === 13 || vo.taskType === 12) {
					...
					//更新互助码 
					  updateAuthorShareCode(vo.assistTaskDetailVo.taskToken,appId )
...
function getAuthorShareCode(url=`${share_code_url}/jd_wish`) {
  return new Promise(async resolve => {
    const options = {
...
function updateAuthorShareCode(code,appid) {
	...
	let url = `${share_code_url}/jd_wish/update?sign=${sign}&username=${$.UserName}&code=${code}&appid=${appid}&time=${time}`;
