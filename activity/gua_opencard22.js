...
!(async () => {
  ...
  $.shareUuid = 'bde38a3d000f4771bd893596ede79cf0'
...
async function run(){
  try{
    ...
    if(!$.vipDraw1) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    if(!$.vipDraw2) await draw(2)
    if(!$.vipDraw2) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    await checkOpenCard();
    console.log(`关注店铺:${$.followshop}`)
    if(!$.followshop) await saveTask('关注店铺', "followshop");
    if(!$.followshop) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    await helpFriend();
    console.log(`签到:${$.signin}`)
    if(!$.signin) await saveTask('签到', "signin&param=");
    if(!$.signin) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    ...
    if(!$.scansku){
      ...
      for (let s of $.getproduct && $.getproduct || []) {
        ...
        if(s.taskDone !== true) await $.wait(parseInt(Math.random() * 1000 + 6000, 10))
      }
    }
    await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    await getActorUuid()
