          ...
          if (err) {
            ...
            if (type) {
			  ...
			  if ( false){
				...
				console.log(`【${buildNmae}】暂停自动升级条件，请手动升级\n`)
			   }
            }
          }
        } catch (e) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `${share_code_url}/jd_cfd`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
...
function uploadShareCode(code) {
	...
	let url = `${share_code_url}/jd_cfd/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
  ...
  function uploadShareCode000(code) {
    return new Promise(async resolve => {
