...
!(async () => {
	...
	try{
      ...
      if (readShareCodeRes && readShareCodeRes.code === 200) {
        $.allInvite = readShareCodeRes.data || [];
      }
    } catch (e) {
      console.log(e);
    }
...
async function doTask(){
    for (let i = 0; i < $.taskList.length; i++) {
        ...
        if($.oneTask.assignmentType === 3 || $.oneTask.assignmentType === 0 || $.oneTask.assignmentType === 1 || $.oneTask.assignmentType === 7){
			...
			updateAuthorShareCode($.oneTask.ext.assistTaskDetail.itemId,$.oneTask.encryptAssignmentId);
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    try{
      const readShareCodeRes = await readShareCode();
      if (readShareCodeRes && readShareCodeRes.code === 200) {
        $.allInvite = [...new Set([...$.allInvite, ...(readShareCodeRes.data || [])])];
      }
    } catch (e) {
      console.log(e);
    }
    console.log(`第${$.index}个京东账号将要助力的好友${JSON.stringify($.allInvite)}`)
    resolve();
  })
}
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `${share_code_url}/jd_tw202201`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function getAuthorShareCode(url = `${share_code_url}/jd_tw202201`) {
  return new Promise(async resolve => {
    const options = {
...
*/
function updateAuthorShareCode(code,appid) {
	let key =  process.env.UPDATE_SHARE_CODE_KEY ?? '';
	let time = Date.now();

	let sign = md5(`username=${$.UserName}&code=${code}&appid=${appid}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_tw202201/update?sign=${sign}&username=${$.UserName}&code=${code}&appid=${appid}&time=${time}`;
