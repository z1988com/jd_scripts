...
!(async () => {
  ...
  $.shareUuid = 'ea25d075a4c3490cb4523d95d250ea15'
...
async function run(){
  try{
    ...
    if($.opencardDraw) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    await getInitInfo();
    await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    ...
    if(!$.signStatus) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
    console.log(`加购商品:${$.skuAddCart}`)
    if(guaopencard_addSku+"" == "true"){
      if(!$.skuAddCart) await saveTask('加购商品', 21, 21);
      if(!$.skuAddCart) await $.wait(parseInt(Math.random() * 1000 + 5000, 10))
