...
async function pasture() {
  try {
    ...
    if (JSON.stringify($.homeInfo) === '{}') {
	  ...
	  updateAuthorShareCode($.homeInfo.sharekey) 
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `${share_code_url}/jd_jxmc`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          //console.log(JSON.stringify(err))
...
function updateAuthorShareCode(code) {
	...
	let sign = md5(`username=${$.UserName}&code=${code}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_jxmc/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
