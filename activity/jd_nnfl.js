...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
		...
		console.log(`====开始【京东账号${$.index}】${$.nickName || $.UserName} 互助====`);
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
		...
		console.log(`====开始【京东账号${$.index}】${$.nickName || $.UserName} 抽奖====`);
        if (rcsArr.includes($.UserName) > 0) {
...
async function UserSignNew() {
    ...
    if (res) {
		...
		updateAuthorShareCode(res.data.token)
...
function updateAuthorShareCode(code) {
	...
	let url = `${share_code_url}/jd_nnfl/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
...
function Env(t, e) {
    ...
    return new class {
        ...
        wait(t) { return new Promise(e => setTimeout(e, t+2000)) }
        done(t = {}) {
