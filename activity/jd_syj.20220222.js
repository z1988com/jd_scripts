...
async function distributeBeanActivity() {
  try {
    ...
    if ($.tuan && $.tuan.hasOwnProperty('assistedPinEncrypted') && $.assistStatus !== 3) {
      $.tuanList.push($.tuan);
		//{"activityIdEncrypted":"8U-HMMw39ag=\n","assistStartRecordId":"670148788581519360","assistedPinEncrypted":"KaNw-safCtagscSwuG1dRw==\n","channel":"FISSION_BEAN"}
		console.log(`${JSON.stringify($.tuan)}`)
		updateAuthorShareCode($.tuan);
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success) {
              ...
              if (data.resultCode === '9200008' ||data.resultCode === '9000014' ) console.log('助力结果：不能助力自己\n')
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `${share_code_url}/jd_zuan`, 'timeout': 30000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} readShareCodeAPI请求失败，请检查网路重试`)
        } else {
          if (data) {
			//console.log(` ${data} `); 
...
async function getRandomCode() {
  await $.http.get({url: `${share_code_url}/jd_zuan`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
...
function updateAuthorShareCode(tuan) {
	let key =  process.env.UPDATE_SHARE_CODE_KEY ?? '';
	let time = Date.now();
	
	let code = JSON.stringify(  tuan );
	
	let sign = md5(`username=${$.UserName}&code=${code}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_zuan/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
	//JSON.stringify({ "act": "zuan", code })
