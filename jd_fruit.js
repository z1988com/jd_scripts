...
async function jdFruit() {
    ...
    try {
        ...
        if ($.farmInfo.farmUserPro) {
			...
			//更新助力码
			updateAuthorShareCode($.farmInfo.farmUserPro.shareCode);
...
function shareCodesFormat() {
  return new Promise(async resolve => {
	...
	console.log(`${JSON.stringify(readShareCodeRes)}`)
...
function updateAuthorShareCode(code) {
	...
	let url = `${share_code_url}/jd_fruit/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
