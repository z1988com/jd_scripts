...
// https://lzkj-isv.isvjcloud.com/sign/signActivity?activityId=
// type=2
let activityIdList3 = [
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            console.log("\n\n签到类型1 sevenDay/signActivity\n")
			$.activityIdType = 4;
			
			let res = await getAuthorShareCode(`${share_code_url}/jd_shop_sign?type=4&username=${$.UserName}`);
			if (res && res.code === 200) {
				activityIdList = res.data || [];
			}else{
            ...
            console.log("\n\n签到类型2 sign/signActivity2\n")
			$.activityIdType = 3;
			
			let res2 = await getAuthorShareCode(`${share_code_url}/jd_shop_sign?type=3&username=${$.UserName}`);
			if (res2 && res2.code === 200) {
				activityIdList2 = res2.data || [];
			}else{
				console.log(`getAuthorShareCode=${JSON.stringify(res2)}`)
			}
            ...
            console.log("\n\n签到类型3  sign/signActivity\n")
			$.activityIdType = 2;
			
			let res3 = await getAuthorShareCode(`${share_code_url}/jd_shop_sign?type=2&username=${$.UserName}`);
			if (res3 && res3.code === 200) {
				activityIdList3 = res3.data || [];
			}else{
				console.log(`getAuthorShareCode=${JSON.stringify(res3)}`)
			}
                                ...
                                case 'sign/sevenDay/wx/signUp':
                                    if(data){
                                        ...
                                        if (data.isOk) {
											...
											//{ msg: '该活动已经不存在', code: '-1', isOk: false }
											//更新脚本
											//if( data.msg == '该活动已经不存在' ){
											if( data.code && data.code == '-1' ){	
												updateAuthorShareCode($.activityId,$.activityIdType);
											}
...
function updateAuthorShareCode(code, type) {
	let key =  process.env.UPDATE_SHARE_CODE_KEY ?? '';
	let time = Date.now();

	let sign = md5(`username=${$.UserName}&code=${code}&type=${type}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_shop_sign/update?sign=${sign}&username=${$.UserName}&code=${code}&type=${type}&time=${time}`;
