/**
 ...
 Date：2021-11-2
先内部助力，有剩余助力作者
1 0,9,19,23 * * * https://raw.githubusercontent.com/ZXX2021/jd-scripts/main/jd_nnfls.js
 */
const $ = new Env('牛牛福利');
...
shareCodes.push('queTyNJUPlCTLWfTWm0mm5bGMXuY2UBilT--RJ6mdYk=');
...
!(async() => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        $.canHelp = true;
        $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
        console.log(`====开始账号【京东账号${$.index}】${$.nickName || $.UserName} 助力===`)
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        $.cookie = cookiesArr[i];
        $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
        console.log(`====开始账号【京东账号${$.index}】${$.nickName || $.UserName} 任务抽奖===`)
        await drawUserTask();
    }
...
async function drawUserTask() {
    ...
    for (let j = 0; j < surplusTimes && coin >= 10; j++) {
        ...
        if (res) {
            ...
            if (res.retCode == 0) {
                console.log('抽奖成功', res.data && res.data.prize && res.data.prize[0]? res.data.prize[0].prizename : "")
            } else {
...
function Env(t, e) {
    ...
    return new class {
        ...
        wait(t) { return new Promise(e => setTimeout(e, t+3000)) }
        done(t = {}) {
