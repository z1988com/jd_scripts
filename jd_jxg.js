...
$.helpCodeList['yanleinet'] = '283b6f75189d48a9b739a813962aca40';
...
!(async () => {
  ...
  for(let i = 0;i<cookiesArr.length;i++){
	...
	let helpCode = $.helpCodeList['yanleinet'];
	if('yanleinet' === $.UserName){
		continue;
	}else{
		console.log(`${$.UserName}助力：${helpCode}`)
		await helpFriend(helpCode);
		await $.wait(3000);
	}
}
...
async function transferBoxCard(cordId){
  ...
  return new Promise(async resolve => {
    $.post(myRequest, (err, resp, data) => {
		console.log(`transferBoxCard=${data}`);
      try {
        data = JSON.parse(data);
