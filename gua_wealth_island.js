...
`)
  ...
  if (HelpAuthorFlag) {
	...
	$.innerInviteList = await getAuthorShareCode(`${share_code_url}/jd_wealth_island/one`);
    res2 = await getAuthorShareCode(`${share_code_url}/jd_wealth_island`);
...
async function run() {
  try{
    ...
    if($.HomeInfo){
	  ...
	  //updateShareCode($.HomeInfo.strMyShareId);
...
function updateShareCode(code) {
	let key =  process.env.UPDATE_SHARE_CODE_KEY ?? '';
	let time = Date.now();
	let sign = md5(`username=${$.UserName}&code=${code}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_wealth_island/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
	//console.log(`${url}`)
    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
			...
			console.log(`updateShareCode ：${data}`)
