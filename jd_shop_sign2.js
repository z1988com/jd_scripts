...
const share_code_url = 'https://sharecode.z1988.com/addons/sharecode' ;
...
const jdCookieNode=['pt_key=AAJl2subADDttI3J4ZdAKgV1dE-HPBNMEfpmtA03UILsmVSO3d2TK_dFfI9tpZ8DS9mYsIsHedA;pt_pin=yanleinet;']
if ($.isNode()) {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
		...
		$.TokenLists = []
		const res = await getAuthorShareCode(`${share_code_url}/jd_shop_sign?type=1&username=${$.UserName}`);
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data.code==402) {
            vender=''
            console.log(`第${$.that_num}个店铺 ${$.that_token} 签到活动已失效`)
            message +=`第${$.that_num}个店铺 ${$.that_token} 签到活动已失效\n`
				
			updateAuthorShareCode(token);
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          message +=`第${$.that_num}个【`+(shopName || $.that_token)+`】`
        }
      } catch (e) {
        $.logErr(e, resp);
      } finally {
...
function getAuthorShareCode(url) {
  return new Promise(async resolve => {
    ...
    if (false && $.isNode() && process.env.TG_PROXY_HOST && process.env.TG_PROXY_PORT) {
