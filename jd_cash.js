...
//[{"inviteCode":"aUNmM6_nOP4j-W4","shareDate":"IRs1be2zZPg"},{"inviteCode":"eU9YO4XYIqdUpw6VqRVV","shareDate":"IRs1be2zZPg"},{"inviteCode":"eU9YaO3mMPV09W-EnXsS1g","shareDate":"IRs1be2zZPg"}]
if ($.isNode()) {
...
!(async () => {
  ...
  await requireConfig()
  //$.authorCode = await getAuthorShareCode(`${share_code_url}/jd_cash`)
  if (!$.authorCode) {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
			  ...
			  //更新助力码
				updateAuthorShareCode(data.data.result.invitedCode )
				//需要添加推送
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `${share_code_url}/jd_cash`, 'timeout': 30000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function updateAuthorShareCode(invitedCode) {
	let key =  process.env.UPDATE_SHARE_CODE_KEY ?? '';
	let time = Date.now();

	let sign = md5(`username=${$.UserName}&code=${invitedCode}&time=${time}&key=${key}`);
	let url = `${share_code_url}/jd_cash/update?sign=${sign}&username=${$.UserName}&code=${invitedCode}&time=${time}`;
