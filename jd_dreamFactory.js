...
!(async () => {
  ...
  await requestAlgo();
  //await requireConfig(); //已经不不需要加载  20220113
  if (!cookiesArr[0]) {
...
async function jdDreamFactory() {
  try {
    ...
    await stealFriend();
    //await tuanActivity(); //活动已经结束 20220113
    //await QueryAllTuan();
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
				...
				//上传互助码
				updateAuthorShareCode(data.user.encryptPin) ;
...
function updateTuanIdsCDN(url = 'http://www.z1988.com/tools/shareCodes/jd_updateFactoryTuanId.php') {
  return new Promise(async resolve => {
    const options = {
...
function readShareCode( url = `${share_code_url}/jd_dream_factory` ) {
  console.log(`开始`)
  //http://share.turinglabs.net/api/v3/jxfactory/query/${randomCount}/
  return new Promise(async resolve => {
...
function updateAuthorShareCode(code) {
	...
	let url = `${share_code_url}/jd_dream_factory/update?sign=${sign}&username=${$.UserName}&code=${code}&time=${time}`;
