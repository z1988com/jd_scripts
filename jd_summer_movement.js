...
$.ShInviteList = [];
$.innerShInviteList = [
  'HcmphKX1H1b2J93PU5h_mmgQiWNVBKqsIUv0dVyMfqZ4r0pJ9Vy7I705mA0jf5I',
];
if ($.isNode()) {
      ...
      if ([1, 3, 5, 7, 9, 26].includes($.oneTask.taskType) && $.oneTask.status === 1) { // 21 开通
        ...
        for (let j = 0; j < $.activityInfoList.length; j++) {
          ...
          if ($.callbackInfo.code === 0 && $.callbackInfo.data && $.callbackInfo.data.result && $.callbackInfo.data.result.taskToken) {
            await $.wait(getRndInteger(5000, 15000));
            let sendInfo = encodeURIComponent(`{"dataSource":"newshortAward","method":"getTaskAward","reqParams":"{\\"taskToken\\":\\"${$.callbackInfo.data.result.taskToken}\\"}","sdkVersion":"1.0.0","clientLanguage":"zh"}`)
            await callbackResult(sendInfo)
          } else if ($.oneTask.taskType === 5 || $.oneTask.taskType === 3 || $.oneTask.taskType === 26) {
            console.log(`任务完成`);
            await $.wait(getRndInteger(4000, 6000));
    ...
    for (let i = 0; i < $.shopInfoList.length; i++) {
      ...
      for (let i = 0; i < $.shopTask.length; i++) {
        ...
        for (let j = 0; j < $.activityInfoList.length; j++) {
          ...
          await takePostRequest('olympicgames_doTaskDetail');
          if ($.callbackInfo.code === 0 && $.callbackInfo.data && $.callbackInfo.data.result && $.callbackInfo.data.result.taskToken) {
            await $.wait(getRndInteger(10000, 15000));
            let sendInfo = encodeURIComponent(`{"dataSource":"newshortAward","method":"getTaskAward","reqParams":"{\\"taskToken\\":\\"${$.callbackInfo.data.result.taskToken}\\"}","sdkVersion":"1.0.0","clientLanguage":"zh"}`)
            await callbackResult(sendInfo)
          } else  {
            await $.wait(getRndInteger(5000, 15000));
