      ...
      result: 0
			...
			*/
            ...
            if (vo.taskName == '每日签到') {
				...
				for (let v of  vo.taskItemList) {
					let date = new Date().Format('yyyy-MM-dd');;
					
					
					if( date == v.itemName){
						console.log(`签到日期:${v.itemName}`);
						await doTask(vo.taskType, v.itemId, vo.id,vo.taskName);
						await getReward(vo.taskType, v.itemId, vo.id);
					}
				}
            }
            if (vo.taskType == 3) {
				//console.log(vo.taskItemList);
				for (let v of  vo.taskItemList) {
					console.log(`浏览会场:${v.itemName}`);
					await getJump(v.itemLink);
					await $.wait(1000 * vo.viewTime +1000)
					await doTask(vo.taskType, v.itemId, vo.id,vo.taskName);
					await getReward(vo.taskType, v.itemId, vo.id);
				}
            }
...
//做任务
function doTask(taskType, itemId, taskid,taskName) {
    return new Promise(resolve => {
